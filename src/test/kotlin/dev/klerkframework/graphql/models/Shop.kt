...
data class Shop(val bestSellers: List<ModelID<Book>>, val faxNumber: FaxNumber?, val shopSize: ShopSize)

enum class ShopStates {
    Deletable,
}

fun shopStateMachine(): StateMachine<Shop, ShopStates, Context, MyViews> = stateMachine {
    event(CreateShop) {}
...
data class CreateShopParams(val faxNumber: FaxNumber?)
